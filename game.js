(() => {
  ...
  const L2 = {
    ...
    collected: 0,
    coinValue: 100,
    priam: { active: false, x: 0, y: 0, vx: -120, hitCD: 0, nextT: 1.4 },
    marcy: { active: false, x: 0, y: 0, vx: -90, nextT: 2.2 },
    stallT: 0,
    collected: 0,
    target: 0,
  };
  ...
  function resetLevel2() {
    ...
    L2.collected = 0;
    L2.coinValue = 100;
    L2.priam = { active: false, x: 0, y: 0, vx: -120, hitCD: 0, nextT: 1.4 };
    L2.marcy = { active: false, x: 0, y: 0, vx: -90, nextT: 2.2 };
    L2.stallT = 0;
    const coinSpots = [
      { x: 300, y: floorY - 100 },
      { x: 360, y: floorY - 100 },
  ...
  // -------------------- Level 2 NPCs (Phase A) --------------------
  function spawnL2Priam(floorY) {
    // King Priam jogs in from the right and wanders across your path
    L2.priam.active = true;
    L2.priam.hitCD = 0;
    L2.priam.x = L2.camX + VIEW.gw + 80;
    L2.priam.y = floorY - 44;
    L2.priam.vx = -(110 + Math.random() * 90);
  }

  function spawnL2Marcy(floorY) {
    // Marcy appears by “trail relay posters” and slows you down if you linger
    L2.marcy.active = true;
    L2.marcy.x = L2.camX + VIEW.gw + 120;
    L2.marcy.y = floorY - 44;
    L2.marcy.vx = -(70 + Math.random() * 70);
  }

  function updateL2NPCs(dt, floorY) {
    // timers
    if (!L2.priam.active) {
      L2.priam.nextT -= dt;
      if (L2.priam.nextT <= 0) {
        L2.priam.nextT = 2.6 + Math.random() * 3.4;
        spawnL2Priam(floorY);
      }
    }
    if (!L2.marcy.active) {
      L2.marcy.nextT -= dt;
      if (L2.marcy.nextT <= 0) {
        L2.marcy.nextT = 3.0 + Math.random() * 4.2;
        spawnL2Marcy(floorY);
      }
    }
    ...
    if (L2.priam.active) {
      L2.priam.x += L2.priam.vx * dt;
      // a little “pace” wobble
      L2.priam.y = (floorY - 44) + Math.round(Math.sin(state.t * 6.0) * 2);
      if ((L2.priam.x - L2.camX) < -80) L2.priam.active = false;
      if (L2.priam.hitCD > 0) L2.priam.hitCD -= dt;
    }
    if (L2.marcy.active) {
      L2.marcy.x += L2.marcy.vx * dt;
      L2.marcy.y = (floorY - 44) + Math.round(Math.sin(state.t * 4.0) * 2);
      if ((L2.marcy.x - L2.camX) < -120) L2.marcy.active = false;
    }

    // proximity stall (Marcy)
    if (L2.stallT > 0) L2.stallT -= dt;

    if (L2.marcy.active) {
      const mx = (L2.marcy.x - L2.camX);
      const my = L2.marcy.y;
      const near =
        Math.abs((player.x + player.w * 0.5) - (mx + 10)) < 70 &&
        Math.abs((player.y + player.h * 0.5) - (my + 18)) < 60;

      if (near) {
        L2.stallT = 0.22; // refresh while close
      }
    }

    // Priam collision: lose 1 coin (and its points)
    if (L2.priam.active && L2.priam.hitCD <= 0) {
      const px = L2.priam.x - L2.camX;
      const py = L2.priam.y;
      if (rectsOverlap(player.x, player.y, player.w, player.h, px, py, 22, 40)) {
        L2.priam.hitCD = 0.6;
        if (L2.collected > 0) {
          L2.collected -= 1;
          L2.score = Math.max(0, L2.score - L2.coinValue);
          SFX.decoy();
          spawnSparkles(player.x + player.w * 0.5, player.y + 10, 12);
        } else {
          // still a little bump feedback even if broke
          SFX.tick();
  ...
  function drawL2Priam() {
    if (!L2.priam.active) return;
    const x = Math.round(L2.priam.x - L2.camX);
    const y = Math.round(L2.priam.y);

    // simple “toga” block
    drawPixelRect(x, y + 16, 22, 22, "rgba(128,0,32,0.95)");
    drawPixelRect(x, y, 22, 16, "#FFD2B5"); // head
    // crown hint
    drawPixelRect(x + 5, y - 2, 12, 4, "#FFD24D");
    drawPixelRect(x + 7, y - 4, 2, 2, "#FFD24D");
    drawPixelRect(x + 13, y - 4, 2, 2, "#FFD24D");
    // “beggar bucket” (for later polish)
    drawPixelRect(x - 6, y + 22, 6, 8, "rgba(0,0,0,0.35)");
  }

  function drawL2Marcy() {
    if (!L2.marcy.active) return;
    const x = Math.round(L2.marcy.x - L2.camX);
    const y = Math.round(L2.marcy.y);

    // simple person + clipboard/poster vibe
    drawPixelRect(x, y + 16, 20, 22, "rgba(255,255,255,0.18)");
    drawPixelRect(x, y, 20, 16, "#FFD2B5");
    drawPixelRect(x, y, 20, 5, "rgba(255,210,77,0.75)");
    // “poster” beside her (placeholder for trail relay signage)
    drawPixelRect(x + 24, y + 6, 10, 28, "rgba(255,255,255,0.14)");
    drawPixelRect(x + 26, y + 8, 6, 2, "rgba(255,210,77,0.35)");
    drawPixelRect(x + 26, y + 12, 6, 2, "rgba(255,255,255,0.20)");
  }
  ...
  function updateLevel2(dt) {
    ...
    // update Trojan Trek NPCs
    updateL2NPCs(dt, floorY);
  ...
  function drawLevel2() {
    ...
    if (L2.stallT > 0) {
      ...
      ctx.textAlign = "center";
      ctx.fillText("Marcy: ‘Trail Relay?’", VIEW.gw / 2, VIEW.gh * 0.18);
    }
